...
function Video() {
    ...
    useEffect(() => {
        console.log(query)
        axios.get(`https://youtube.googleapis.com/youtube/v3/search?part=id&maxResults=1&q=${query}&regionCode=US&key=${YOUTUBE_API.API}`)
        .then(res => {
    ...
    const printOutput = () => {
        if (video!== undefined){
            return video.list.items.map(e=>(
                <iframe width="1280" height="400" src={`https://www.youtube.com/embed/${e.id.videoId}`} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            ))
        } else return
    }
